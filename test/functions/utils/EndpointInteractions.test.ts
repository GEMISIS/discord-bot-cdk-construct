...
describe('Test EndpointInteractions', () => {
  ...
  test('Test successful followup', async () => {
    ...
    expect(
        await EndpointInteractions.sendFollowupMessage(endpointInfo, token, responseData),
    ).toBeTruthy();
    expect(mockedAxios.post).toBeCalledTimes(1);
    expect(mockedAxios.post).toBeCalledWith(
        expect.stringContaining(`https://discord.com/api/v10/webhooks/appid/token`,
    ...
    expect(mockedAxios.post).toBeCalledWith(
        expect.stringContaining(`https://discord.com/api/v9/webhooks/appid/token`,
        ), expect.anything(), expect.anything());
  });
  ...
  test('Test unsuccessful followup', async () => {
    ...
    mockedAxios.post.mockImplementationOnce(
        (_url: string, _data?: any, _config?: AxiosRequestConfig) => {
          throw new Error();
        });
    expect(
        await EndpointInteractions.sendFollowupMessage(endpointInfo, token, responseData),
